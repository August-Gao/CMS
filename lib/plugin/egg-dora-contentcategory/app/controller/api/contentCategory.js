...
const ContentCategoryController = {
  ...
  async getChildrenCategoriesById(ctx, app) {
    try {
      ...
      for (const item of cates) {
        const cateContents = await ctx.helper.reqJsonData(
          `content/getList?typeId=${item._id}&isPaging=0`
        );

        item.contents = cateContents;
      }
